...
public function buscar(Request $request)
{
    ...
    if ($request->filled('q')) {
        ...
        $query->where(function ($builder) use ($q) {
            $builder->where('titulo', 'like', "%{$q}%")
                    ...
                    ->orWhere('id', 'like', "%{$q}%"); // busca por id si el usuario pone un número
        });
    }
    //  NUEVO: si viene el parámetro mas_vistos, ordenar por vistas descendente
    if ($request->boolean('mas_vistos')) {
        $query->orderBy('vistas', 'desc');
    } else {
        $query->orderBy('id', 'desc'); // orden normal
    }
    ...
    // Filtro por capacidad (texto parcial)
    if ($request->filled('capacidad')) {
        $query->where('capacidad', 'like', '%' . $request->capacidad . '%');
    }
