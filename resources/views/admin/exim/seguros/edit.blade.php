...
@section('title', 'Editar Seguro - EXIM')

@section('content')
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h3 class="fw-bold">Editar Seguro</h3>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-body">
            <form action="{{ route('admin.exim.seguros.update', $seguro) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <input type="text" name="nombre" class="form-control @error('nombre') is-invalid @enderror"
                               value="{{ old('nombre', $seguro->nombre) }}" required placeholder="Ej: Seguro Marítimo Básico">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" step="0.01" name="porcentaje" class="form-control @error('porcentaje') is-invalid @enderror"
                               value="{{ old('porcentaje', $seguro->porcentaje) }}" min="0" required>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="moneda_id" class="form-select @error('moneda_id') is-invalid @enderror">
                            ...
                            @foreach($monedas as $moneda)
                                <option value="{{ $moneda->id }}" {{ old('moneda_id', $seguro->moneda_id) == $moneda->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" step="0.01" name="costo_base" class="form-control @error('costo_base') is-invalid @enderror"
                               value="{{ old('costo_base', $seguro->costo_base) }}" min="0" required>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="activo" class="form-select @error('activo') is-invalid @enderror">
                            <option value="1" {{ old('activo', $seguro->activo ? '1' : '0') == '1' ? 'selected' : '' }}>Activo</option>
                            <option value="0" {{ old('activo', $seguro->activo ? '1' : '0') == '0' ? 'selected' : '' }}>Inactivo</option>
                        </select>
