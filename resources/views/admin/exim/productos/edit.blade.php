...
@section('title', 'Editar Producto - EXIM')
...
<div class="container-fluid py-4">
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-body">
            <form action="{{ route('admin.exim.productos.update', $producto) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <select name="tipo" class="form-select @error('tipo') is-invalid @enderror" required>
                            <option value="cafe_verde" {{ old('tipo', $producto->tipo) === 'cafe_verde' ? 'selected' : '' }}>Café Verde</option>
                            <option value="cafe_tostado" {{ old('tipo', $producto->tipo) === 'cafe_tostado' ? 'selected' : '' }}>Café Tostado</option>
                            <option value="cafe_molido" {{ old('tipo', $producto->tipo) === 'cafe_molido' ? 'selected' : '' }}>Café Molido</option>
                            <option value="maquinaria" {{ old('tipo', $producto->tipo) === 'maquinaria' ? 'selected' : '' }}>Maquinaria</option>
                            <option value="equipo_industrial" {{ old('tipo', $producto->tipo) === 'equipo_industrial' ? 'selected' : '' }}>Equipo Industrial</option>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="producto_id" class="form-select @error('producto_id') is-invalid @enderror">
                            ...
                            @foreach($productosLocales as $prod)
                                <option value="{{ $prod->id }}" {{ old('producto_id', $producto->producto_id) == $prod->id ? 'selected' : '' }}>
                                    {{ $prod->titulo }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" step="0.01" name="precio_base" class="form-control @error('precio_base') is-invalid @enderror"
                               value="{{ old('precio_base', $producto->precio_base) }}" min="0" required>
                    ...
                    <div class="col-md-4">
                        <div class="form-check mt-4">
                            <input type="checkbox" name="estado" class="form-check-input" value="1" id="estado" {{ old('estado', $producto->estado ? '1' : '0') ? 'checked' : '' }}>
